...
namespace KinematicCharacterController
{
    ...
    public struct HitStabilityReport
    {
        ...
        public bool ValidVaultDetected;
        public Collider VaultedCollider;
...
#pragma warning restore 0649
        ...
        /// </summary>    
        [Tooltip("Maximum height of a step which the character can climb")]
        public float MaxVaultHeight = 2f;
        /// <summary>
...
#pragma warning restore 0414 
        ...
        public void EvaluateHitStability(Collider hitCollider, Vector3 hitNormal, Vector3 hitPoint, Vector3 atCharacterPosition, Quaternion atCharacterRotation, Vector3 withCharacterVelocity, ref HitStabilityReport stabilityReport)
        {
            ...
            if (StepHandling != StepHandlingMethod.None && !stabilityReport.IsStable)
            {
                ...
                if (!(hitRigidbody && !hitRigidbody.isKinematic))
                {
                    ...
                    DetectVault(atCharacterPosition, atCharacterRotation, hitPoint, innerHitDirection, ref stabilityReport);

                    Debug.DrawLine(stabilityReport.VaultPosition, stabilityReport.VaultPosition + Vector3.up, Color.red);
        ...
        private void DetectSteps(Vector3 characterPosition, Quaternion characterRotation, Vector3 hitPoint, Vector3 innerHitDirection, ref HitStabilityReport stabilityReport)
        {
            ...
            if (CheckStepValidity(nbStepHits, characterPosition, characterRotation, innerHitDirection, stepCheckStartPos, out tmpCollider, MaxStepHeight, ref stepPosition))
            {
        ...
        private void DetectVault(Vector3 characterPosition, Quaternion characterRotation, Vector3 hitPoint, Vector3 innerHitDirection, ref HitStabilityReport stabilityReport)
        {
            int nbStepHits = 0;
            Collider tmpCollider;
            RaycastHit outerVaultHit;
            Vector3 characterUp = characterRotation * _cachedWorldUp;
            Vector3 verticalCharToHit = Vector3.Project((hitPoint - characterPosition), characterUp);
            Vector3 horizontalCharToHitDirection = Vector3.ProjectOnPlane((hitPoint - characterPosition), characterUp).normalized;
            Vector3 vaultCheckStartPos = (hitPoint - verticalCharToHit) + (characterUp * MaxVaultHeight) + (horizontalCharToHitDirection * CollisionOffset * 3f);
            ...
            nbStepHits = CharacterCollisionsSweep(
                            ...
                            characterRotation,
                            -characterUp,
                            MaxVaultHeight + CollisionOffset,
                            out outerVaultHit,
                            _internalCharacterHits,
                            0f,
                            true);
            ...
            // Check for overlaps and obstructions at the hit position
            if (CheckStepValidity(nbStepHits, characterPosition, characterRotation, innerHitDirection, vaultCheckStartPos, out tmpCollider, MaxVaultHeight, ref vaultPosition))
            {
                stabilityReport.ValidVaultDetected = true;
                stabilityReport.VaultedCollider = tmpCollider;
                stabilityReport.VaultPosition = vaultPosition;
            }

            if (StepHandling == StepHandlingMethod.Extra && !stabilityReport.ValidVaultDetected)
            {
                // Do min reach step check with capsule cast on hit point
                vaultCheckStartPos = characterPosition + (characterUp * MaxVaultHeight) + (-innerHitDirection * MinRequiredStepDepth);
                nbStepHits = CharacterCollisionsSweep(
                                vaultCheckStartPos,
                                characterRotation,
                                -characterUp,
        ...
        private bool CheckStepValidity(int nbStepHits, Vector3 characterPosition, Quaternion characterRotation, Vector3 innerHitDirection, Vector3 stepCheckStartPos, out Collider hitCollider, float maxHeight, ref Vector3 stepPosition)
        {
            hitCollider = null;
            ...
            while (nbStepHits > 0 && !foundValidStepPosition)
            {
                ...
                if (atStepOverlaps <= 0)
                {
                    ...
                    if (CharacterCollisionsRaycast(
                            ...
                            -characterUp,
                            maxHeight + SecondaryProbesVertical,
                            ...
                            true) > 0)
                    {
                        if (IsStableOnNormal(outerSlopeHit.normal))
                        {
                            ...
                            if (CharacterCollisionsSweep(
                                                ...
                                                maxHeight - farthestHit.distance, // distance
